 ...
 * Sample Output-1:
 * ---------------
 * [AACBCB, ABCACB, ACBABC, ABACCB, ACABCB, AABCCB, ACABBC, AACBBC, ABACBC,
 * AABCBC]
 * 
 * 
 * Sample Input-2:
...
public class InterleavingStringsFormattedInput {
    ...
    public static void interleave(String X, String Y, String current, List<String> result) {
        if (X.length() == 0 && Y.length() == 0) {
            result.add(current);
            return;
        }
        if (X.length() > 0) {
            interleave(X.substring(1), Y, current + X.charAt(0), result);
        }
        if (Y.length() > 0) {
            interleave(X, Y.substring(1), current + Y.charAt(0), result);
        }
    }
}
